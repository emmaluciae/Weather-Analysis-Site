...
def home():
    ...
    result = {
        ...
        "desc": wx_desc(c.get("weathercode", -1)),
        "time": c.get("time"),
        ...
        # Today’s snapshot (first element if available)
        "tmax_today": (d.get("temperature_2m_max") or [None])[0],
        "tmin_today": (d.get("temperature_2m_min") or [None])[0],
        "uv_today":   (d.get("uv_index_max") or [None])[0],
        "prcp_today": (d.get("precipitation_sum") or [None])[0],

        # Hourly series
        "hourly_temp": f.get("hourly", {}).get("temperature_2m", []),
        "hourly_time": f.get("hourly", {}).get("time", [])
    }

    return render_template("index.html", result=result, error=None)
@app.route("/night")
def night():
    # For now just render the template — later we’ll pull real data
